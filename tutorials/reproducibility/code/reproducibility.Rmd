...
> An abstract of an Essay on the Origin of Species... blaaaaah.
> Writing is hard. Maybe I'll watch Netflix instead.
>
...
1) In GitKraken, `[File icon in upper left] -> Clone`
...
3) In your browser, navigate to the repository and

<!--TODO TODO TODO: uh-oh. make sure this is as simple as it should be. dammit, windows.-->
...
Finally, the most specific comments are **in-line comments**. As a beginning programmer, you should probably use in-line comments liberally, to make explicit to yourself what each line does. However, as you gain experience, inline comments should be used sparingly, if at all. There are a few reasons to avoid in-line comments once you gain experience coding:
 ...
 2) Similarly, if your code is written well, then the comment should be redundant
...
>
> When is the distribution modeled in Exercise 5 approximately normally distributed?
>
> Make a figure analogous to Figure 1, but with newly generated data nicely approximated by a normal distribution.
>
> Make sure you comment your code and include all the information necessary to reproduce your figure.
...
> **Exercise 7**: 
>
>A Poisson distribution has a variance-to-mean ratio equal to 1. Show that the variance of the spider count distribution is approximately equal to its mean.
...
Check visually how the distributions of the three bug types fit to Poisson and LGP distributions.

1) Plot the Poisson fit to the *probability* of spider counts, along with the data
2) Plot the Poisson fit to the *probability* of sowbug counts, along with the data
3) Plot the Poisson fit to the *probability* of weevil counts, along with the data
4) Add a curve to Plot 1) showing the LGP fit to the *probability* of spider counts
5) Add a curve to Plot 2) showing the LGP fit to the *probability* of sowbug counts
6) Add a curve to Plot 3) showing the LGP fit to the *probability* of weevil counts

Each figure should have datapoints corresponding to empirical data, and lines corresponding to theoretical distribution. Indicate in the README how well the each distribution fits these data sets.

For LGP, you can use the function `dLGP` from the library `RMKdiscrete`

For sowbugs, you can use the empirically determined `lambda2 = 0.53214` and estimate `lambda1` by the formula `lambda1 = mean * (1 - lambda2)`. 

### The requirements

1) Create a **private** repository on GitHub.
      - On the GitHub homepage, select "New Repository" and then on the next page select "Private"
2) Add all members of your group as collaborators, plus Graham (username: grahamas) and Stephanie (usernames: sepalmer).
